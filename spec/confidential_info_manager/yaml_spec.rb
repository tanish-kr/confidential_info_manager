...
describe ConfidentialInfoManager::YAML do
  ...
  context "save to YAML, read" do
    let!(:secret_data) { { API_KEY: "abcedefg", API_SECRET_KEY: "abcedfg"} }
    let!(:file_path) { File.join(File.dirname(File.expand_path(__FILE__)), "secret_data.yml") }
    let!(:pass) { "pass" }
    ...
    it "saving the encrypted data in YAML format" do
      ...
      confidential_info_manager = described_class.new(pass)
      confidential_info_manager.save(secret_data, file_path)
      expect(File).to exist(file_path)
    end

    it "reads the load encrypted YAML file, decryption" do
      confidential_info_manager = described_class.new(pass)
